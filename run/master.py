...
class App(Tk):
    ...
    def __init__(self, auto_update, OS):
        ...
        if auto_update == 1:
            LOGGER('update', 'master').info(
                'Начинаем процесс проверки обновлений'
            )
    ...
    def initialize_ui(self):
        ...
        self.tk.call(
            'wm', 'iconphoto', self._w, self.app_ico['LittlePassHTML_ico']
        )
    ...
    def build_settings(self):
        ...
        update_frame.grid(row=0, column=0, sticky='NWE', padx=5)
        ...
        theme_frame.grid(row=2, column=0, sticky='NWE', padx=5)
        import_pass_frame = ttk.Frame(self.settings)
        ...
        ttk.Label(
            theme_frame, text='Тема оформления HTML:',
            font=('Times New Roman', 13, 'bold italic')
        ...
        ttk.Label(
            password_frame, text='Пароль дешифровки:',
            font=('Times New Roman', 13, 'bold italic')
        ...
        autoupdate_on = ttk.Radiobutton(
            update_frame, text='Включено',
            value=1, variable=self.auto_update_var
        )
        autoupdate_off = ttk.Radiobutton(
            update_frame, text='Отключено',
            value=0, variable=self.auto_update_var
        )
        ttk.Button(
        ...
        ttk.Label(
            import_pass_frame, text='Импортировать пароли:',
            font=('Times New Roman', 13, 'bold italic')
        ).grid(row=0, column=0, sticky='NWE')
        ttk.Button(
            import_pass_frame, text='Из CSV',
            command=lambda: self.import_passwords_from_file(0)
        ).grid(row=0, column=1, sticky='NWE', padx=5)
        ttk.Button(
            import_pass_frame, text='Из passinhtml',
            command=lambda: self.import_passwords_from_file(1)
        ).grid(row=0, column=2, sticky='NWE')
        ttk.Button(
            import_pass_frame, text='Из иного файла',
            command=lambda: self.import_passwords_from_file(-1)
        ).grid(row=0, column=3, sticky='NWE', padx=5)

        ttk.Label(
            export_pass_frame, text='Экспортировать пароли:',
            font=('Times New Roman', 13, 'bold italic')
        ).grid(row=0, column=0, sticky='NWE')
        ttk.Button(
            export_pass_frame, text='В passinhtml (только для приложения)',
        ...
        autoupdate_on.bind(
            '<Button-1>', lambda event: self.click_to_radio_auto_update()
        )
        autoupdate_off.bind(
            '<Button-1>', lambda event: self.click_to_radio_auto_update()
        )
    ...
    def click_to_radio_auto_update(self):
        ...
        if var == 0:
            ...
            self.loading_set.after(
                4000, lambda: self.loading_set.grid_remove()
            )
    ...
    def generate_main(self):
        ...
        left_frame = ttk.Frame(
            self.main, relief='solid', borderwidth=1, padding=5
        )
        ...
        self.search = ttk.Entry(
            top_frame, width=73, font=('Times New Roman', 12)
        )
        ...
        generate = ttk.Button(
            right_top_frame, text='Генерировать', width=13, cursor='star'
        )
        add = ttk.Button(
            right_center_frame, text='Добавить', width=13, cursor='plus'
        )
        self.edit = ttk.Button(
            right_center_frame, text='Редактировать', width=13,
            cursor='pencil', state='disable'
        )
        self.delete = ttk.Button(
            right_bottom_frame, text='Удалить', width=13,
            cursor='circle', state='disable'
        )
        self.delete_all = ttk.Button(
            right_bottom_frame, text='Удалить все', width=13,
            cursor='pirate'
        )
        ...
        self.list_password.bind(
            '<<ListboxSelect>>', lambda event: self.enable_button_touch()
        )
        self.list_password.bind(
            '<Double-Button-1>', lambda event: self.edit_password()
        )
        self.list_password.bind(
            '<Delete>', lambda event: self.delete_password()
        )
        self.delete.bind(
            '<Button-1>', lambda event: self.delete_password()
        )
        self.delete_all.bind(
            '<Button-1>', lambda event: self.delete_all_passwords()
        )
        add.bind('<Button-1>', lambda event: self.add_password())
        self.edit.bind(
            '<Button-1>', lambda event: self.edit_password()
        )
        btn_search.bind(
            '<Button-1>', lambda event: self.completetion_into_listbox(True)
        )
