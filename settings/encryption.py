...
class Encryption:
    def __init__(self):
        ...
        self.alphabet, self.step = self.settings['alphabet'],  \
            self.settings['step']
    ...
    def create_passwords_settings():
        ...
        for i in range(len(alphabet)):
            symbol = alphabet[i]
            encrypt_alphabet[symbol] = i + 1
            reversed_encrypt_alphabet[i + 1] = symbol
    ...
    def encrypt(self, text: str):
        ...
        for symbol in text:
            ...
            while number_symbol > length:
                number_symbol -= length
            while number_symbol < 1:
                number_symbol += length

            if self.step > 0 and number_symbol > length:
                number_symbol = 1
            if self.step < 0 and number_symbol == 0:
                number_symbol = length
